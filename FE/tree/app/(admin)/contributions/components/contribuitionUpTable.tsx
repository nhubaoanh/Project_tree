...
interface ContributionTableProps {
  ...
  onEdit: (contribution: IContributionUp) => void;
  onDelete: (contribution: IContributionUp) => void;
  selectedIds?: number[];
...
export const ContributionTable: React.FC<ContributionTableProps> = ({
  ...
  const allSelected = data.length > 0 && data.every((e) => selectedIds.includes(e.thuId));
  const someSelected = data.some((e) => selectedIds.includes(e.thuId));
              ...
              ? data.map((item, index) => (
                  <tr
                    ...
                    className={`hover:bg-[#fffdf5] transition-colors group ${
                      selectedIds.includes(item.thuId) ? "bg-[#fff8e1]" : ""
                    }`}
                  >
                    <td className="p-4 w-12 text-center">
                      <input
                        type="checkbox"
                        checked={selectedIds.includes(item.thuId)}
                        onChange={(e) => onSelectOne?.(item.thuId, e.target.checked)}
                        className="w-4 h-4 accent-[#b91c1c] cursor-pointer"
                      />
                    </td>
